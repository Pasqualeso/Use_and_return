var carrello = new Array();    /* carrello con gli acquisti. Vettore di oggetti contenenti codice, prezzo, quantità
                                           di ciascun prodotto */
...
function cambia(cella) {
    /* una delle quantità è cambiata aggiorna le variabili */
    var label = "q"+cella;
...
function tabella() {
    document.write("<TABLE border=1><TH>Codice<TH>Titolo<TH>Data di inizio<TH>Data di fine<TH>Descrizione<TH>Prezzo x giorno<TH>Giorni<TH>Totale\n ");
    for(var i=0; i<carrello.length; i++) {
...
function svuota() {
    ...
    document.getElementById('elenco').innerHTML =
        "<TABLE border=1><TH>Codice<TH>Titolo<TH>Data di inizio<TH>Data di fine<TH>Descrizione<TH>Prezzo x giorno<TH>Giorni<TH>Totale</TABLE>";
}
